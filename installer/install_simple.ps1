...
if ($Uninstall) {
    ...
    Write-Ok "done. (data dir at $env:APPDATA\bastion\bastion preserved — delete manually if you want a clean slate)"
    return
}
...
if (-not (Test-Path $SourceExe)) {
    throw "source binary not found at $SourceExe — build it with: cd agent ; cargo build --release"
}
...
if (Test-Path $tokenPath) {
    ...
    Write-Host ""
    Write-Host "Paste the token into the BASTION desktop app (one time — it persists in localStorage)."
} else {
    Write-Warn2 "token file not found yet — give it a few more seconds, then check $tokenPath"
}
...
try {
    ...
    Write-Warn2 "port 7878 not responding yet — may still be starting"
}
